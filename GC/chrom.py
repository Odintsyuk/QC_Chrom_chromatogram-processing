...
# value - словарь, в котором:
        # keys - элементы списка parameters,
        # values - значения
components = {}
parameters = ['t, c',
              'H, пA',
...
# представление данных хроматограммы в формате: [мин:сек, сигнал]
        ...
        for i in range(2, len(L) - 2, 10):
                ...
                try:
                        m_s_format = datetime.time(minute=int((i - 2) / 10) // 60, second=int((i - 2) / 10) % 60).strftime(
                                '%M:%S')
        ...
        for i in t:
                if abs(time_ethanol - i) < abs(time_acn - i):
                        ...
                        print('Необходимо уточнение компонента, '
                              'включите ручной режим и обработайте пики')
                else:
                        time_acn = i
